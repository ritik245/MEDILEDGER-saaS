...
import { signup, signin, uploadRecord, downloadRecord } from './api';

const roles = ['PATIENT', 'DOCTOR', 'ADMIN'];

function App() {
  const [authMode, setAuthMode] = useState('signin');
  const [authForm, setAuthForm] = useState({ name: '', email: '', password: '', role: 'PATIENT' });
  const [token, setToken] = useState(localStorage.getItem('mediledger.token') || '');
  const [user, setUser] = useState(JSON.parse(localStorage.getItem('mediledger.user') || 'null'));
      ...
      <main className="grid">
        <section className="panel">
          <div className="panel-title-row">
            ...
            <button className="ghost" onClick={() => setAuthMode((m) => (m === 'signin' ? 'signup' : 'signin'))}>
          ...
          <form onSubmit={handleAuth} className="stack">
            {authMode === 'signup' && (
              <>
                <label>Name<input name="name" value={authForm.name} onChange={onChange} required /></label>
                <label>Role
            ...
            <label>Email<input name="email" type="email" value={authForm.email} onChange={onChange} required /></label>
            <label>Password<input name="password" type="password" value={authForm.password} onChange={onChange} required /></label>
            <button type="submit" className="primary">{authMode === 'signin' ? 'Authenticate' : 'Create user'}</button>
          </form>
        </section>
        ...
        <section className="panel">
          ...
          <form onSubmit={handleUpload} className="stack">
            ...
            <button type="submit" className="primary" disabled={!token}>Upload to IPFS</button>
          </form>

          <form onSubmit={handleDownload} className="stack compact">
            <label>Download by record ID
              <input value={recordId} onChange={(e) => setRecordId(e.target.value)} placeholder="e.g. 1" />
            </label>
            <button type="submit" className="primary" disabled={!token}>Download Record</button>
          </form>

          <button className="danger" onClick={handleSignout}>Clear Session</button>
        </section>
      </main>
